...
def tradutor(palavra):
    from string import ascii_uppercase as traduz
    resp = ''
    for c in palavra:
        resp += traduz[ordem.index(c)]
    return resp

txtA.sort(key=tradutor)
txtB.sort(key=tradutor)
...
for palavra in txtA:
    if len(palavra) == 7 and palavra[-1] not in zumbi:
        contVerbA += 1
        if palavra[0] not in zumbi:
            priPessoaA.append(palavra)

for palavra in txtB:
    if len(palavra) == 7 and palavra[-1] not in zumbi:
        contVerbB += 1
        if palavra[0] not in zumbi:
            priPessoaB.append(palavra)

def calcula(palavra):
    valor = 0
    for x in range(len(palavra)):
        valor += ordem.index(palavra[x]) * 20 ** x
    return valor

for palavra in txtA:
    if len(set(palavra)) == len(palavra) and calcula(palavra) % 42 == 0:
        magicosA.append(palavra)

for palavra in txtB:
    if len(set(palavra)) == len(palavra) and calcula(palavra) % 42 == 0:
        magicosB.append(palavra)
        
print ('=================== QUESTÃO A ===================')
print (f'TextoA = {contadorA}\nTextoB = {contadorB}')
print ('=================== QUESTÃO B ===================')
print (f'TextoA = {contVerbA}\nTextoB = {contVerbB}')
print ('=================== QUESTÃO C ===================')
print (f'TextoA = {len(priPessoaA)}\nTextoB = {len(priPessoaB)}')
print ('=================== QUESTÃO D ===================')
print (f'TextoA = {txtA}')
print (f'TextoB = {txtB}')
print ('=================== QUESTÃO E ===================')
print (f'Mágicos do TextoA = {magicosA}\nMágicos do TextoB = {magicosB}')
print (f'Quantidade dos mágicos do TextoA = {len(magicosA)}\nQuantidade dos mágicos do TextoB = {len(magicosB)}')
